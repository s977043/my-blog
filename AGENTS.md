...
同一プラットフォーム内の自記事へのリンクは**内部リンク**であり、次節のクロスプラットフォーム参照（外部リンク）とは扱いが逆になる。**末尾のリンク集に加えて、本文中の文脈内にも置く**。

- 目安: 本文中の文脈内リンクを 2〜4 本（長文でも増やしすぎない）。末尾の `## 参考` / `## 関連記事` はそれとは別に置いてよい
...
**なぜ外部リンクと逆なのか**: 同一ドメイン内のリンクは評価がサイト内で再分配されるが、他プラットフォームへのリンクは評価を外へ渡す。前者は文脈内に置く価値があり、後者は末尾に留めて本数を絞るほうが合理的。
...
この規約は読者体験を理由に定めたものだが（PR #92 の 2 段階修正）、SEO 上も整合する。外部リンクは記事あたり 1〜3 本に絞るのが一般的な目安であり、「文脈内配置のほうが評価される」という指針は**内部リンクの話**なので、本規約を緩める根拠にはならない。
