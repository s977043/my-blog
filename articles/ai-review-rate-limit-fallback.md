...
ただ、1 体の AI に見てもらうだけだと、見落としやバイアスが残ります。そこで「複数の AI にレビューさせればよい」と考えるわけですが、実際にやってみると別の壁にぶつかります。

**レート制限です。**

Codex CLI や Gemini CLI のような外部 AI レビュー CLI を複数つないで多視点レビューを回そうとすると、429、認証エラー、trusted-directory 系のエラーなどで簡単に止まります。

この記事では、私が実際に AI レビュー運用で詰まった経験をもとに、**AI レビューをレート制限で止めないための可用性設計**を整理します。

## 結論

AI レビューは、精度だけでなく**可用性**で設計する必要があります。
...
この記事では、AI レビューを 2 つの軸で整理します。

1 つ目は、**可用性**です。レビューが落ちずに回るか。落ちたとしても判断を止めずに済むか、という観点です。

2 つ目は、**独立性**です。同じ見落としや同じバイアスを避けられるか、という観点です。

この 2 つは似ていますが、同じものではありません。
...
私の運用では、あるスクリプトのレビューで、1 つの独立 AI だけが致命的な穴を実証付きで発見したことがありました。自分のセルフレビューでは気づけませんでした。

この経験から、AI レビューでは「賢さ」だけでなく、**独立した視点をどう作るか**が重要だと感じています。

ただし、独立性を上げるために外部 AI を増やしていくと、今度は可用性の問題が出ます。

## 実際に起きたこと

ある変更を、複数の AI でレビューしようとしたときのことです。

対象は CI のガードスクリプトでした。自分のレビューに加えて、Claude、Codex、Gemini の複数視点で確認しようとしました。
...
このとき、比較的安定して使えたのは PR に紐づく bot レビューでした。

ここで気づいたのは、**外部 CLI を品質ゲートの主経路に置くと、レビュー内容以前の理由で止まる**ということです。

レビューは品質ゲートです。外部サービスのレート制限や認証状態でゲートが開かなくなるなら、それは品質保証ではなく運用リスクになります。
...
なお `trusted-directory` 系のエラーは、非対話実行（CI 等）では信頼確認をスキップして回避できる場合があります。手元の `gemini-cli` v0.42 では `--skip-trust` フラグか `GEMINI_CLI_TRUST_WORKSPACE=true` 環境変数で通りました（フラグ名・要否はバージョン依存なので公式ドキュメント要確認）。
...
特にセキュリティ観点では、AI レビューは補助として扱い、CI 上の機械的な検査と併用するべきです。
:::
...
ただし、外部 CLI を主経路にすると、レート制限や認証エラーで簡単に止まります。

そのため、AI レビューは次のように設計するのが現実的です。

- L1: 自前で起動・制御しやすい独立エージェント
- L2: PR 連動の bot レビュー
- L3: Codex CLI / Gemini CLI などの外部 CLI
- L3 は落ちる前提で扱う
- L1 + L2 が揃えば、L3 を待たずに判断できるようにする
- skip ログを残し、レビュー済み範囲を明確にする
