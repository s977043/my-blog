...
でも「多視点でレビューさせよう」と外部 AI を複数つないだ瞬間、現実が牙をむきます。**レート制限（429）です。**
...
- **Gemini CLI**: `pro` モデルが `429 You have exhausted your capacity on this model`。さらに、非対話実行が `trusted-directory` エラーで弾かれる（自分の環境では信頼確認をスキップするフラグが必要だった。フラグ名・要否はバージョン依存なので公式ドキュメント要確認）。`flash` に切り替えると動くが、所見が浅く「問題なし」しか返らない。
...
ポイントは、**最も可用な L1 を自前の Claude Code 内で完結させ、L2 を PR 連動の安定枠、L3 を「回ればラッキー」の補助枠**として扱っていることです（L3 が両方落ちても判断を止めない扱いは前節の degradation の通り）。
...
# - 非対話では信頼確認をスキップするフラグが要る環境がある
# - フラグ名は仮（下記 --skip-trust は実在名ではなく例示。実際の Gemini CLI のフラグ名はバージョン依存。公式ドキュメントで確認すること）
# - pro が 429 のときは flash に逃がす（ただし所見は浅くなる前提）
# - レビュー指示と対象ファイルを ``` で囲んで stdin に流すだけ
( cat review-instructions.md; echo '```'; cat target.js; echo '```' ) \
  | gemini --skip-trust -m gemini-2.5-flash   # --skip-trust は仮の例示。実フラグ名は要確認
```
