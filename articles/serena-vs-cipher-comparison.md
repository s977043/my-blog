...
**Serena（CLIオンデマンド）**  
![Serenaのアーキテクチャ（CLI型）](/articles/images/serena-architecture.png)  
*開発者 → ターミナル/CLI → Serena（都度スキャン／コンテキスト生成）→ LLM。要点：導入が簡単／軽量／CLI中心。*

**Cipher（常駐＋メモリレイヤー）**  
![Cipherのアーキテクチャ（常駐・メモリレイヤー）](/articles/images/cipher-architecture.png)  
...
![選択フローチャート：個人×CLIはSerena、IDE／複数人／大規模はCipher](/articles/images/serena-vs-cipher-chooser.png)
