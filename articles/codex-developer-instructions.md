...
:::message
この記事では、Codex の挙動を単発プロンプトではなく判断ルールで安定させる考え方、`developer_instructions` と `AGENTS.md` の役割分担、実務向けテンプレートとアンチパターンを整理します。
:::
...
ただし、`developer_instructions` の指定方法や名称は利用環境によって異なります。
...
`developer_instructions` は、Codex の振る舞いを制御するための判断ルールです。
...
`AGENTS.md` のようなプロジェクトローカルな指示とは役割が違うため、AI エージェントの挙動に強い影響を与えます。

## ざっくりした設計モデル

内部実装の正確な優先順位を外部から断定することはできません。
そのためこの記事では、実務上は次のように分けて考えます。
