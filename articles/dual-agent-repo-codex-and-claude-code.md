...
A. 現状の結論は「Codex 固有のスキルが無いなら `.agents/skills` を削除する（本文の方針3: 削除＋AGENTS.md集約）」が最もシンプルです。Windows 開発者がいる場合のシンボリックリンクのリスクを考えると、削除 + `AGENTS.md` への規約集約が安全です。固有スキルが必要になった時点で改めて分離を検討します。
