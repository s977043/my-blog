...
A. `@testing-library/user-event` v14 以降 + Jest 29 以降が前提です（2026年時点では Jest 30 系も同じAPIで動作確認済み）。それ以前のバージョンでは `userEvent` が同期APIだったり `advanceTimers` オプションが無かったりするため、package.json のバージョンを先に確認します。動かない場合は `act` で明示的に `jest.advanceTimersByTime` をラップする旧来の書き方にフォールバックします。
