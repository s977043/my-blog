...
| :--- | :--- | :--- | :--- | :--- |
| **Cliff (崖)** | 失敗＝致命傷。厳密な検証。 | 0.0 - 0.1 | **事前承認必須** | 認証、決済、セキュリティ、DB移行 |
| **Hill (丘)** | 修正可能なミス。定型作業。 | 0.3 - 0.5 | 事後レビュー (PR) | リファクタリング、命名規則、テスト記述 |
| **Plain (平原)** | 探索的な思考。自由な発想。 | 0.7 - 0.9 | 承認不要 | ブレスト、モック作成、リスク洗い出し |
