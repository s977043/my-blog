...
- 導入が **3 ステップのコマンド**で済むようになり、テンプレコピーや `AGENTS.md` の手配置、`skills/` のコピー作業が消えた。
...
- **手作業のフォールバックは残す**：オフライン環境向けに v0.x 相当の経路は残す（詳細は後述）
...
「マーケットプレイスを追加 → プラグインを入れる」という同じ流れが Claude Code と Codex で揃っているのがポイントです（Claude Code は最後に `/reload-plugins` でリロードする一手間が入りますが、Codex 側は install と同時に有効化されます）。

> リリースタグでバージョンを pin する運用は、Claude Code なら `/plugin install river-review@v1.2.1`、Codex なら `codex plugin marketplace add s977043/river-review@v1.2.1` のようにタグを付けます。README にも明記してあり、プロダクションでは pin が安全です。
...
同じスキルパックを使えるとはいえ、**何に向くか**は違います。マーケットプレイスとバージョニングは Claude Code / Codex で共通なので、ユースケース側の差だけを表に整理します。
