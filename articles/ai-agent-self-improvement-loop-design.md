...
**想定読者**: Claude CodeやCodexなどのAIコーディングエージェントを日常運用し、Skills・Hooks・CIを育てている人
**前提**: 実測値と実装例はClaude Code中心です（設計原則は他エージェントにも読み替え可能。詳細は「はじめに」の注記を参照）
:::
...
- Skills一覧は約250エントリ。一覧に載る名前・説明文の合計文字数は、切り詰め前の原文を独自集計すると、コンテキストへ常駐させるには明らかに過大な規模（概算）
- 累計622セッションで1〜2回しか使われていないSkillが19個常駐
- 同じ目的のSkillが3系統に重複

Claude Codeは、利用可能なSkill名と説明の一覧をコンテキストへ読み込みます。[公式ドキュメント](https://code.claude.com/docs/en/skills)には、各Skillの説明は一覧上1,536文字で切り詰められると記載されています。一方、一覧全体の肥大化がSkill選択へ与える影響は公式仕様として明記されているわけではなく、次に述べるのは私の環境での観測です。切り詰めの具体値はバージョンによって変わりうるため、固定値は運用基準にしていません。
...
Claude Code Hooksは、特定のライフサイクルイベントで処理を実行する仕組みです。自然言語の指示より決定的な制御に向きますが、発火するイベントと実行環境の範囲を超えては強制できません。ブロックの伝え方には規約があり（`PreToolUse`では`exit 2` + stderr、または`exit 0`でdenyを返す構造化JSONのどちらか一方。`exit 1`はブロックになりません）、詳細は[Hooksガイド](https://code.claude.com/docs/en/hooks-guide)と[Hooksリファレンス](https://code.claude.com/docs/en/hooks)を参照してください。
