...
- 記録・棚卸し・機械化・剪定の4機能を、作業の反復回数とリスクに応じて分岐させる判断基準
...
- Skills一覧は約250エントリ。各Skillの名前と説明文を合計すると、一覧用に割り当てられる文字数予算の約4〜5倍（切り詰め前の原文を独自集計した概算）
- 累計622セッションで1〜2回しか使われていないSkillsが19個常駐
- 同じ目的のSkillsが3系統に重複

Claude Codeは、利用可能なSkill名と説明の一覧をコンテキストへ読み込みます。一覧が文字数予算を超えると説明が短縮され、呼び出し判断に必要なキーワードが失われる可能性があります。これは[公式ドキュメント](https://code.claude.com/docs/en/skills)にも記載されています。具体的な予算や切り詰め方はバージョンによって変わるため、固定値は運用基準にしていません。
...
| 機械化 | 注意力頼みのルールを実行時制御へ移す | 事故・未遂・高リスク検出時／棚卸し時 | Hooks、チェッカー、CI |
...
| 重大事故・高リスク操作を防ぐガード | 期待損失と可逆性 | 待たない |
...
Claude Code Hooksは、特定のライフサイクルイベントで処理を実行する仕組みです。自然言語の指示より決定的な制御に向きますが、発火するイベントと実行環境の範囲を超えては強制できません。ブロックの伝え方には規約があり（`PreToolUse`では`exit 2` + stderr、または構造化JSONのどちらか一方。`exit 1`はブロックになりません）、詳細は[Hooksガイド](https://code.claude.com/docs/en/hooks-guide)と[Hooksリファレンス](https://code.claude.com/docs/en/hooks)を参照してください。
...
方針として、判定ロジックは独自チェッカーへ集約し、複数のレイヤーから同じチェッカーを呼び出せる形を目指しています。私の環境で完成しているのはGit操作前の検証で、同じ検証スクリプトをnpm script（手動・CI用）とClaude Codeの`PreToolUse` Hook（push・PR操作の直前ガード）が共用しています。Hookでは非ゼロ終了を`exit 2`へ変換するだけです。レイヤーごとに同じ条件を実装し直さずに済み、判定のずれを抑えられます。
...
私の環境では、2026年7月の棚卸し時点で、Claude Codeがローカルに保持する統計とセッションログを集計し、Skillごとの累計使用回数と最終使用時期を確認しました（622セッションは計測時点までの累計）。これは公式に保証された分析機能ではないローカル情報の独自集計なので、数値は自環境の参考値です。
