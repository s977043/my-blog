...
私の環境では、2026年7月の棚卸し時点で、Claude Codeがローカルに保持する情報とセッションログを集計し、累計使用回数や最終使用時期を確認しました。622セッションは計測時点までの累計です。本記事では、Skillの明示的または自動的なロードが記録された回数を「使用回数」として扱っています。ただし、同一セッション内の再呼び出しを含む集計粒度は、使用したログや内部情報の形式に依存します。これはSkill単位の利用分析を保証する公式機能でも、製品間で比較できる標準指標でもありません。そのため、ここで示す数値は自分の環境で観測した参考値として扱っています。
