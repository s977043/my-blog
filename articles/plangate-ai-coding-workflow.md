...
### Q. PlanGate はスクラムでない開発（カンバン、個人開発）にも使えますか？
A. 使えます。本質は「計画承認をAI実行のゲートにする」ことで、PBI という用語をタスクや Issue に読み替えれば成立します。Acceptance Criteria と Out of scope を書く習慣さえあれば、スクラムのセレモニーは不要です。

### Q. `plan.md / todo.md / test-cases.md` などファイルが多すぎませんか？1ファイルにまとめてはダメ？
A. まとめてもワークします。ただし分けている理由は、AIが各フェーズで参照する範囲を限定するためです。1ファイルにすると、AIが exec フェーズで plan 全体を読み直して計画を勝手に書き換える事故が起きやすくなります。最初は薄くてもファイル分割を維持するのが安全です。

### Q. レビューを `review-self.md` と `review-external.md` で二重に書くのは冗長では？
A. 役割が違うので二重ではありません。self はAI自身に計画の弱点を出させるためのもので、external は人間が承認前に見る外部視点のチェックです。selfで見落としを潰してから external に渡すことで、人間レビューの負荷が下がります。

### Q. セッションが切れても再開できると書かれていますが、AIが status.md を信じてくれない場合は？
A. 再開時のプロンプトで「まず `status.md` と `todo.md` を読み、phase と next を宣言してから作業を始めて」と明示します。会話履歴に頼らず、ファイルから状態を復元する手順を冒頭に固定するのが定着のコツです。

### Q. PlanGate を導入してから AI 作業が遅くなりませんか？
A. 1タスクあたりの初動は確かに遅くなります。ただし「スコープ逸脱した実装の巻き戻し」「セッション切れによる再説明」が消えるため、3日以上にまたがるタスクではトータルで速くなります。1日で終わる軽量タスクはオーバーヘッドが上回るため、PlanGate を通さない判断もあり得ます。
...
:::message
- 📕 [Zenn Book: AI にコードを書かせる前にやること — PlanGate 実践ガイド](https://zenn.dev/minewo/books/plangate-guide) - この記事の考え方を、計画→実装→検証→チーム展開の型として全9章で体系化した実践ガイドです。
- [PlanGate v3 から v8.6 までの設計変遷：AI コーディング統制ハーネスはどう育ったか](https://zenn.dev/minewo/articles/plangate-design-evolution-v3-to-v8) - この記事の考え方がバージョンを重ねてどう育ったかの設計変遷です。
- [AIコーディングを「比較で改善」できる土台にする：PlanGate v8.6.0のMetrics v1とGovernance](https://zenn.dev/minewo/articles/plangate-v86-hook-enforcement) - 最新版で入れた計測と統制の話です。
