...
emoji: "🚦"
type: "tech"
topics: ["ai", "claudecode", "codex", "githubactions", "automation"]
published: true
---
...
- v8.5.0 で **Hook enforcement 10/10** が揃い、「止める」仕組みは出揃いました
- v8.6.0 で **Harness Improvement Roadmap Phase 0/1**（baseline → governance → privacy → Metrics v1）が乗り、「観測する・比較する」と合流しました
- 11 events の JSON Schema、CLI（`plangate metrics --collect/--report`）、CLI tests 24→32 PASS（hook 側は 42 PASS 維持）の詳細は本文で整理します

hook violation や C-3/C-4 の判断を週次で振り返りたいチームに向いています。「AIに速く書かせる」だけでなく、「AIがどこで止まり、どれくらい止まっているか」を数字で扱いたいチームに合います。
...
| v8.5.0 | Hook enforcement 完成 | 10/10 hooks 実装、hook tests 12→42 PASS |
...
EPIC #193 [Harness Improvement Roadmap](https://github.com/s977043/plangate/issues/193) の v8.6.0 milestone P0 として、4 つの PBI が、**Metrics v1 の前に baseline・governance・privacy を整える依存関係で**実装されました。
...
設計時にこの線引きが入っているので、Metrics v1 を使っても、Forbidden カテゴリを意図せず保存しにくい構造になっています。
...
ここまで触ってから Hook と Metrics に進むのが現実的です。最初から `metrics --collect` を回しても、observe する対象（TASK 履歴）が無いと意味がありません。
...
### Q. Metrics v1 はダッシュボード機能ですか？

いいえ。v8.6.0 時点の Metrics v1 は、ダッシュボードというより **後続改善を比較するためのイベントログ基盤**です。

`plangate metrics --collect` で TASK から events を抽出し、`--report` や `--aggregate` で集計します。見た目の可視化よりも、「どの hook が何回止めたか」「C-3 / C-4 の判断がどう推移したか」を後から比較できることを優先しています。

### Q. Hook enforcement だけ入れていれば十分ですか？

最初の導入では Hook enforcement だけでも十分です。

ただし、運用を続けると「止められたか」だけでなく、「どこで何回止まったか」「それは改善しているか」を見たくなります。Metrics v1 は、その振り返りを retrospective や週次レビューに乗せるためのレイヤーです。

### Q. Privacy の線引きはどこで確認できますか？

`docs/ai/metrics-privacy.md` が正本です。

保存可能 12 カテゴリと禁止 9 カテゴリを先に決め、その禁止カテゴリを schema 上に持たせない設計にしています。自チームで使う場合も、まずこの privacy policy を自分たちの公開・保存ルールに照らして確認するのがおすすめです。

### 新規導入では、v8.6.0 のどこから触るべきですか？

まずは 1 TASK で `plan → approve → exec` を試すのがよいです。

Metrics v1 は TASK 履歴があって初めて意味を持ちます。最初から全部を入れるより、PlanGate の承認フローを 1 周してから、`metrics --collect --dry-run` で events 抽出を確認する順序が現実的です。
...
- 📕 [Zenn Book: AI にコードを書かせる前にやること — PlanGate 実践ガイド](https://zenn.dev/minewo/books/plangate-guide) - 計画→実装→検証→チーム展開の型を全9章で体系化した実践ガイドです。
- [アジャイルでAI駆動開発をどう回すか: PlanGateの考え方とテンプレート](https://zenn.dev/minewo/articles/plangate-ai-coding-workflow) - PlanGate の考え方の入門編です。
- [PlanGate v3 から v8.6 までの設計変遷：AI コーディング統制ハーネスはどう育ったか](https://zenn.dev/minewo/articles/plangate-design-evolution-v3-to-v8) - v8.6.0 に至るまでの設計変遷です。
- [AIエージェント3種で戦略議論したら、OSSロードマップの主軸が根本から変わった話](https://zenn.dev/minewo/articles/multi-ai-discussion-roadmap-rewrite) - v8.7 以降のロードマップを組み替えた意思決定の記録です。
- [Qiita: AIの止まり方を「数字で見る」ようにした体験：PlanGate v8.6.0でMetrics v1とGovernanceを入れた話](https://qiita.com/s977043/items/5ebff79112ecf1af872c)
