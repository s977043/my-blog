...
冒頭の3点（正本の固定・接続方法の吸収・役割分担）は、設計としては最初から見えていました。実際に運用して効いたのは、むしろその周辺です。
...
そして、これらを保ったのは**週次の昇格**という地味なリズムです。最初から完璧な記憶を書かず、`00 Inbox` に雑に貯めて、何度も参照する前提だけを `08 Agent Context` に引き上げる。淘汰を前提にしたことで、運用が止まりませんでした。
