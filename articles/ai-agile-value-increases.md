...
たとえば従来なら「2つの設計案のどちらが良いか」を議論で決めていた場面で、両方を小さく実装して実際に動かし、比べてから選ぶ、という進め方が現実的になります。判断の根拠が「議論」から「動く実物」に変わる。これは仮説検証のサイクルが一段速くなるということです。
...
AIは実装を高速化します。しかし、システム全体の整合性や設計の妥当性を自動的に保証してくれるわけではありません。たとえば、目の前の機能だけ見れば妥当なコードでも、既存のドメインモデルや責務分担と食い違ったまま量産されると、後から戻すコストはむしろ増えます。速く作れることが、そのまま負債を速く積むことにもなり得ます。
...
ただし、ここで強調したいのは、実践の形が変わっても本質は動かないということです。先に述べたとおり、本質は「短いフィードバックループを通じて学習し、継続的に価値を最大化すること」であり、AIはその学習サイクルを高速に回すための技術です。

その結果として、スプリントの長さ、スクラムイベントの進め方、振り返りの観点、メトリクスの設計は大きく変わるかもしれません。
...
要件定義や設計の重要性が増すこと、そして実装・テスト・修正のコストが下がること。この2つは矛盾しません。前者は「何を作るべきか」の精度を上げ、後者は「速く試して学ぶ」回転を上げる。どちらもアジャイルの強み——短いフィードバックループで学習し続けること——を後押しします。

だからこそ結論は、ウォーターフォールへの回帰ではありません。スプリントの周期、振り返りの手法、計測・評価の仕方といった実践は変わっていくはずです。

それでも、変わらない本質があります。

それは、仮説を立て、小さく作り、学び、方向修正しながら、継続的に価値を最大化することです。

AIはアジャイルを置き換える技術ではありません。アジャイルの学習サイクルを、これまで以上に速く、強く回すための技術です。

「AIで開発はウォーターフォールに戻る」のではなく、「AIでアジャイルはようやく本領を発揮する」。私はそう考えています。
