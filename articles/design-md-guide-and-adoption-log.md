...
### Q. DESIGN.md と Figma / Penpot はどう使い分ける？

A. DESIGN.md はデザインツールの代替ではありません。Figma や Penpot が「見た目の正本」、DESIGN.md は「実装時にどのファイルを正本として参照し、どう検証するか」を示す入口です。役割が重ならないので併用が前提になります。

### Q. DESIGN.md と README / CONTRIBUTING.md は何が違う？

A. README はプロジェクト全体の入口、DESIGN.md は UI 実装判断に特化した入口です。「token の正本はどこか」「外部 UI 語彙をどう正規化するか」など、デザイン実装に固有の参照順だけを扱う点が違います。

### Q. デザイナーがいないチームでも DESIGN.md は有効？

A. むしろ AI 実装中心の小規模チームほど効きます。判断基準を人の記憶ではなくテキストに固定することで、レビュー観点と AI への指示の入口が揃うためです。
