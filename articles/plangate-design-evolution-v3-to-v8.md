...
|---|---|---|
| ゲート設計 | 計画承認後に進むゲート型モデル | C-3 三値 + V-1〜V-4 + Hook 10/10 + Metrics |
...
| 契約と検証 | Markdown ベースの規約 | Artifact schema 7 種を起点に、model / eval / event schema と CI 検証へ拡張 |
...
- **#198 Keep Rate v1** — v8.6 の Metrics v1 events と baseline が前提（Code / Plan / Acceptance / Handoff の保持率を後続で定義）
- **#199 Dynamic Context Engine v1** — v8.3 の Prompt Assembly 4 層を「実行時に組み立て直す」拡張

ここでは本稿の 4 軸に直結する 4 件だけを取り上げました。CHANGELOG v8.6.0 の Next EPIC には他にも、v8.7.0 の **#203 Tool Error Taxonomy and Recovery Policy** / **#204 PlanGateBench Fixture Suite**、v8.9.0 の **#200 Reporting & Retrospective** が並んでいます。
...
- ゲート設計：ゲート型モデル → C-3 三値 + V-1〜V-4 → Hook 10/10 → Metrics
- モード分類：2 分類 → 5 段階の方向性 → GatePolicy → Workflow DSL → validate --mode
- 実行層：3 層分離 → CLI → Provider RFC → Orchestrator
- 契約と検証：Markdown 規約 → Artifact schema 7 種 → CI 統合 → Outcome-first → event schema
