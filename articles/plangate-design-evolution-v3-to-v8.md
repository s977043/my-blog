...
v3.0.0（2026-04-09）で AI 駆動開発ワークフローの基盤を出してから、v8.6.0（2026-05-04）で Metrics v1 と Governance が揃うまで、約 1 か月で 22 リリースを刻んできました。
...
- バージョン別年表（v3 から v8.6 まで 22 リリース）
...
|---|---|---|
| ゲート設計 | C-3 二値（APPROVE / REJECT） | C-3 三値 + V-1〜V-4 + Hook 10/10 + Metrics |
| モード分類 | 「ライト / フル」の 2 分類 | 5 モード × GatePolicy × Workflow DSL |
| 実行層 | Plan / ToDo / Test Cases の手書き運用 | Workflow YAML + CLI + Provider RFC + Orchestrator |
| 契約と検証 | Markdown ベースの規約 | JSON Schema 7 種 + Schema validate CI + Outcome-first contract |

「止めるための仕組み」と「止めたことを観測する仕組み」が、別々に育って v8.6 で合流しました。
...
### v3.0.0（2026-04-09）— 二値ゲートの導入

- PBI から Plan / ToDo / Test Cases を生成する基本フローを定義
- 計画承認後に Agent 実行へ進む **ゲート型モデル** を初めて導入
- ただしこの時点での C-3 は APPROVE / REJECT の二値しかなく、「条件付き承認」が表現できなかった

### v4.0.0（2026-04-09）— C-3 三値化と V-1〜V-4 の輪郭

- C-3 を **APPROVE / CONDITIONAL / REJECT** の三値に整理
...
- v6.0.0 で Context Engineering と 18 エージェント体制、5 段階モード分類を導入
...
- v8.4.0 で **3 mode hook 設計**（default warning / strict block / bypass escape）を導入し、3 つの hook（EH-2 / EHS-2 / EHS-3）を実装
...
- ライト / フルの 2 分類から、`ultra-light / light / standard / high-risk / critical` の 5 段階へ
- ただしこの時点では、モードと適用ゲートの対応はドキュメント表のみ
...
### v8.0.0 〜 v8.1.0 — Provider RFC と外部エージェント統合
...
## v8.7 以降に向けて何が前提になったか

v8.6.0 完了時点で、Roadmap 上の次の 4 つは「比較可能な状態」を前提にしています。

- **#196 Eval comparison for harness changes** — v8.4 の eval runner と v8.6 の baseline が前提
- **#197 Model Profile v2** — v8.3 の 4 profile と Prompt Assembly が前提
- **#198 Keep Rate v1** — v8.6 の Metrics v1 events が前提（pr_created / handoff_completed イベントから派生）
- **#199 Dynamic Context Engine v1** — v8.3 の Prompt Assembly 4 層を「実行時に組み立て直す」拡張
...
- ゲート設計：二値 → 三値 → V-1〜V-4 → Hook 10/10 → Metrics
- モード分類：2 分類 → 5 段階 → GatePolicy → Workflow DSL → validate --mode
- 実行層：3 層分離 → CLI → Provider RFC → Orchestrator
- 契約と検証：Markdown 規約 → JSON Schema → CI 統合 → Outcome-first → Metrics privacy
