...
:::details Q. 今から PlanGate を導入するなら、どのバージョンから入ればいい？
現行の到達点である **v8.6 系を起点**にするのが素直です。ただし全機能を一度に入れる必要はありません。「承認ゲート（v3〜v4 相当）→ モード分類 → schema 検証 → hook / metrics」の順で段階導入するのが、書き直しを避ける王道です。
:::

:::details Q. v3〜v7 の情報はもう古いですか？読む価値はありますか？
機能としては v8.6 に統合済みですが、**「なぜその順序で積み上がったか」を理解するには各軸の v3〜v7 の経緯が前提**になります。この記事の核心も「機能の総量ではなく入れる順序が品質を決める」点にあるので、変遷そのものに価値があります。
:::

:::details Q. 全部入れないと使えませんか？最小構成は？
いいえ。**Markdown の承認ゲート（v3〜v4 相当）だけで成立**します。hook や metrics は後付けで構いません。むしろ観測・強制の対象が固まる前にそれらから作ると、後で書き直しになります。
:::

:::details Q. Claude Code 以外（Codex / Gemini CLI など）でも使えますか？
v8.0〜v8.1 の Provider RFC で**実行プロバイダを差し替え可能**になりました。承認ゲート・モード・schema 検証といった統制の骨格は、特定のエージェントに依存しません。
:::

:::details Q. なぜ hook や metrics を最初に作ってはいけないのですか？
**観測対象・強制対象が未定義のまま実装に着手すると、後で全部書き直しになる**からです。先に「何を止めるか（契約）」「どう分けるか（モード）」を固定し、最後に「どう観測するか（metrics）」を載せる順序が、手戻りを最小化します。
:::
