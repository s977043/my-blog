---
title: "AI駆動開発の2層ガード設計：PlanGateとRiver Reviewで実装前後を守る"
emoji: "🛡️"
type: "tech"
...
River Review は、暗黙知を再現可能な `Agent Skills` に変えることを目指した AI コードレビューのフレームワークです。
...
River Review では、これをスキルとして明示化し、GitHub Actions や CLI などで再利用できる形に寄せています。
...
River Review 的な運用で大事なのは、レビュー観点を大きくしすぎないことです。
...
PlanGate と River Review を別々に使うだけでも意味はあります。
...
ここで大事なのは、PlanGate の成果物を River Review の入力として扱えることです。

- PlanGate の `目的` は、レビュー時の「この変更で何を守るか」になる
- PlanGate の `スコープ外` は、余計な変更を検知する観点になる
- PlanGate の `テスト観点` は、River Review の Verify に接続できる
...
PlanGate と River Review を1つの巨大なAIワークフローにまとめたくなるかもしれません。
...
だから、PlanGate は計画の承認に集中し、River Review は差分の評価に集中させる。
...
実務では、PlanGate で決めた基準を River Review で確認し、失敗したら次の計画やスキルに戻す、という3つの局面で分けると扱いやすくなります。
...
River Review で、変更内容をスキルベースで評価します。
...
River Review側に戻すなら、スキルに止める条件を追加します。
...
| 4日目 | AIに実装させる | PR |
| 5日目 | River Review相当の観点でレビューする | Review結果 / Verify |
...
ただし、例外もあります。
すでに開発フローは安定していて、レビュー観点だけが属人化しているチームなら、River Review から入る価値があります。
...
| 実装前の合意が曖昧 | PlanGate |
| レビュー指摘が人によって違う | River Review |
| 毎回同じレビュー漏れがある | River Review |
| セッションごとに判断が消える | 両方 |
...
この問いに答えられる形でPlanGateとRiver Reviewを見ると、単なる便利ツールではなく、AI駆動開発の運用部品として扱いやすくなります。
...
PlanGate は着手前のゲートを作り、River Review はレビューの再現性を高めます。
...
- [River Review Docs](https://river-review.the3396.com/)
