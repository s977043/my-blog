...
1. `plan`（計画フェーズ）で変更計画を作る
2. 人間がレビューする
3. `exec`（実装フェーズ）で実装に進む

> `plan` / `exec` の実コマンド（Slash Command）と成果物テンプレートは [アジャイルでAI駆動開発をどう回すか: PlanGateの考え方とテンプレート](https://zenn.dev/minewo/articles/plangate-ai-coding-workflow) を参照してください。
...
River Reviewer は、暗黙知を再現可能な `Agent Skills`（レビュー観点を再利用可能な単位に切り出したもの）に変えることを目指した AI コードレビューのフレームワークです。詳細な設計思想は [AIエージェントを"投げっぱなし"にしない：Agent Skillsと自由度の設計で実現する「評価駆動の開発エコシステム」](https://zenn.dev/minewo/articles/zenn-river-reviewer-architecture) で解説しています。
...
中心にあるのは、リスクに応じて AI の自由度を設計し、`Plan（計画）/ Validate（ゲート承認）/ Verify（自動検証）` でレビュー運用を分ける考え方です。人間の承認点を残したまま AI の再現性を上げられるので、レビューを単なるコメント生成ではなく、**組織の知識を育てる工程**として扱えます。
