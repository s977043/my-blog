---
title: "Zenn Bookを多層AIレビューで作ったら、『収束』の先に実バグが残っていた話"
emoji: "📕"
type: "tech"
...
- 全9章の Zenn Book を、**役割を分けた多層レビュー**（セルフ／役割別 AI レビュー3視点／別モデル査読［Codex・Gemini］／実レンダリング検証／最終追加レビュー）で何度も見直して公開した。
- 各レビュー層は**別々の種類の問題**を捕まえた。とくに「**Zenn が markdown 画像の SVG を表示しない**」という公開ブロッカーは、レビュー文章でなく**実レンダリング検証（Playwright で実際の表示を確認）**でしか出てこなかった。
- AI レビューが「もう直すところはない（収束）」と言った後に走らせた**追加の網羅レビュー**が、それまですり抜けていた実バグ（**コピペで動かないサンプル**等）を掘り当てた ―― いずれも公開前に解消できた。
- 同時に、**改善提案を敵対的に採否判断する**ことで「水増し改善」を却下できた。レビューは足し算でなく、**足すべきものと却下すべきものを分ける**営みだった。
...
flowchart TD
    ...
    S & M & C & G & R --> U[追加の網羅レビュー<br/>章別並列+敵対的な採否]
    U --> P[公開判断]
```
...
| 追加の網羅レビュー | 収束後の見逃し＋過剰研磨の抑制 |

ポイントは、**同じ AI に何度も読ませても同じ盲点が残る**ことです。執筆・修正に使った AI と検証する AI を分け、さらに別系統（Codex/Gemini）を当てることで、思い込みが突き崩されます。

## 層ごとに「違う問題」が出た

### 公開ブロッカーは、文章レビューでは出なかった

最初、図を見栄えのする SVG で用意していました。文章レビューでは誰も問題にしませんでした。ところが **Playwright で Zenn のプレビューを実際にレンダリング**したところ、Zenn のレンダラがこう出力しました。
...
**Zenn は markdown 画像の SVG を表示しません**（対応フォーマットは PNG/JPEG/WebP/GIF）。一方で **mermaid のコードブロックは描画されます**（公開時点の Zenn で確認）。これは「文章を読む」レビューでは絶対に出てこない、**実際に動かして初めて分かる**種類の問題でした。

→ 図はすべて mermaid に置き換え、解消。教訓は「**最終出力の媒体で、実際にレンダリングして確かめる**」。なお mermaid 化は副次的にも相性がよく、**画像ファイルの管理が不要**になり、**テキストなので後から AI で修正しやすい**（AI 執筆との親和性が高い）という利点がありました。

> 補足：表示確認は `zenn preview`（ローカルプレビュー）を Playwright でレンダリングして行い、公開後に本番（zenn.dev）でも目視しました。

### 「もう収束した」あとに、追加レビューが実バグを掘り当てた

多層レビューを数ラウンド回し、複数のレビューが「これ以上は水増し」と判断する**収束状態**に達しました。普通ならここで公開です。

ですが、ダメ押しで**章ごとに並列でレビューし、外部リンクの実在確認や改善提案の敵対的な採否判断まで行う追加レビュー**を走らせたところ、**収束済みの本でも見逃されていた実バグ**が出ました。代表例：
...
面白かったのは、この追加レビューが**改善提案を却下もした**ことです。各提案を「これは本当に読者価値を上げるか、それとも水増しか」と敵対的に検証し、たとえば次を**却下**しました。
