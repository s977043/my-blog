...
| `docs/article-graph.{json,md}` | 生成物（`article_seeds/` だけを投影する）。`article_seeds/` 配下を追加・変更したら `npm run build:article-graph` で再生成して同じ PR に含める。検証の `check:article-graph` は `npm run check` に入っておらず CI でだけ走るため、ローカルの check が通っても CI で落ちる（2026-09-24 #693） |
...
- **「現状はこうなっている」という前提説明が誤っていたら、その旨を報告に明記する。** 黙って正しい方に直して進めない。オーガナイザーは誤った前提のまま次の判断（他タスクの委託・ユーザーへの報告）を組み立てているので、訂正が伝わらないと誤りが横へ広がる。2026-09-09 のウェーブでは2件同時に起きた: ①「`articles_note/` は除外リストに入っている」→ 実装は `DOCS_DIR = "docs"` で**走査対象ですらなかった** ②「記録側と比較側の両方を正規化せよ」→ 記録側は `.claude/` 配下にあり、同じ委託文の「`.claude/` に触るな」と衝突していた
- **委託文の指示どうしが衝突していたら、境界（触ってよい範囲）を優先し、衝突と採った方針を報告に書く。** 目的を満たすために境界を越えない。越える必要があると判断したら、実行せず報告して指示を仰ぐ
- **lint / チェッカーの WARN を「全部消すべき指摘」として扱わない。** 検出結果を「直す / 意図的で残す / チェッカーの誤検出」に**先に分類**し、分類結果を報告に書く。全件を機械的に潰すと、強調・リズム・定義句のような意図的な表現まで壊れる（2026-09-10: 記事の語尾を28件すべて敬体化して単調になり、外部レビューで指摘されて戻す作業が発生した。次の記事では先に分類し、副作用を 1/5 に抑えられた）。残す判断をしたものは**なぜ残すか**を報告に書く
- **副作用が測れる変更は、前後を測って報告に載せる。** 「直した」だけでは、直しすぎたかどうかを受け手が判断できない
