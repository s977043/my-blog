...
- `[ready-to-publish]` **#12 (note) 締切 2026-09-03**: 「個人のAI開発ナレッジを、チームの改善ループへ育てるまで」（PR #575 で主題を「昇格→ループ→ハーネス」へ再構成しタイトル変更。旧題「AI駆動開発を「個人技」で終わらせない。チームの仕組みに変えるまで」）（`articles_note/new/PRONI-plangate-team-rollout.md`（2026-09-09 に会社公式へ転換）。PR #524 マージ済み・構成レビュー完了〔P1/P2 指摘なし〕）。一次情報: Growth-Teams-Agent の `docs/team-onboarding/CHANGELOG.md`・`improvement-backlog.md`〔FB-001/031/032〕・`.agents/metrics/`、plangate README。**2026-09-10 更新**: 会社公式（PRONI）へ転換したことで①は解消した（`AGENTS.md` §題材と媒体の対応 で会社リポジトリ × note 会社公式 は掲載可。GTA の内部実数と固有名はそのまま残している）。ペルソナ13名レビュー・Grok レビュー・語尾統一を反映済みで、WXR は `import-PRONI-plangate-team-rollout-20260910-1711.xml` が最新。**残るのは社内ゲートのみ**: ①カバー画像（デザイン室テンプレート） ②エンジニアレビュー2名（部長 must + グループ内1名 must） ③広報の事前確認依頼ワークフロー（内部実数の粒度はここで判断） ④note公開時に目次をON ⑤ASCII図の実表示確認（2026-08-27 に code block の最大表示幅を 67→38 に圧縮済み。崩れる場合は全体図のみ画像化）
...
- `[dropped]` **#11 (zenn) 2026-09-10 破棄**: 「worktree 分離だけでは防げない、並列AIセッションのGit事故を事後検知で機械化する」（`articles/parallel-session-git-accident-detection.md`。原稿は完成していたが削除。git 履歴から取り出せる）。**破棄理由**: 題材が my-blog 発で、2026-09-08 に成文化した `AGENTS.md` §題材と媒体の対応（my-blog 発の題材は記事にしない）に抵触する。題材差し替えを検討し growth-lab の同型事故を照合したが、報告された3件のうち2件が実物と一致せず（#1002 は base=develop・3ファイルで150ファイル衝突の痕跡なし、#1007 と #1015 は触るファイルが重複しない）、成立しなかった。加えて記事後半の検知系（`check-pr-staleness.sh` と self-test 9ケース、`scripts/hooks/`）は my-blog にしか存在せず、題材を差し替えると主眼の「事後検知で機械化する」が丸ごと消える。**再開する場合**: 他リポジトリで同型の事故と検知系が揃ってから書き直す。theme-discovery スコアは 17/20 で市場の空白も確認済みだった
...
- 2026-09-09 zenn agent-utilization-vs-flow-efficiency https://zenn.dev/minewo/articles/agent-utilization-vs-flow-efficiency （queue 外。theme-discovery 20/20 で起案し、当日中に企画→執筆→レビュー→公開まで通した。**HTTP 200・Zenn API 出現を確認済み**〔id 648446、published_at 2026-09-09 15:57 JST〕。**前回公開〔2026-09-06 06:37〕から 81.3 時間**空けたため rate-limit に触れず、マージ直後に反映。公開3点セット: 初稿 #632 → 再検証反映 #634 → flip #633 main 段 → sync #635 release/zenn でマージし deploy 発火。一次情報は個人リポジトリ3つ〔growth-lab / PlanGate / river-review〕の実測のみで、`AGENTS.md` §題材と媒体の対応 に従い会社リポジトリと my-blog は使っていない。`/review-improve-loop` を2回〔loops 3 + 1、サブエージェント16体〕。1回目は Loop3 で must/high=0 により早期終了、2回目の最終 Verify が出した high 1件〔在庫側には留保を徹底しているのに PR 側の「速くなった」だけベースラインが無い内部不整合〕を #634 で解消した。レビューで訂正した事実誤り4件: 「97本が一度も push されていない」の過剰断定〔`comm -23` では判定できない。`merge-base --is-ancestor` で 97 = 31 取り込み済み + 66 未統合 に切り分けて実測〕/ 関連記事リンクの 404 / Scrum Guide 2020 の誤引用〔WIP 制限は Kanban Guide for Scrum Teams 側〕/ 上記のベースライン不在。**sync 時の注意 2 件**: ①`sync-release-zenn.sh` の公開影響プレビューが 0 件と誤表示した〔`false→true` の遷移のみ数え、新規ファイルが最初から `true` のケースを検知しない〕ため `published:` を全件手動突合した ②release/zenn に main へ存在しないファイルが 13 件蓄積しており、AGENTS.md §単方向不変条件に反していたため main 側採用で削除した〔うち 2 件は PR #626 の `proni-`→`PRONI-` リネームの削除側が未伝播。sync スクリプトは `articles_note/drafts/` の rename/rename しか自動解決しない〕）
- 2026-09-06 zenn ai-second-brain-multi-agent-memory https://zenn.dev/minewo/articles/ai-second-brain-multi-agent-memory （queue #15、締切 2026-08-31 以降。**HTTP 200・Zenn API 出現を確認済み**〔id 618264、published_at 2026-09-06 06:37 JST〕。公開2点セット: flip #601 main 段〔`publishing` state を新設して初適用〕→ sync #613 release/zenn でマージし deploy 発火。**前回公開〔2026-09-04 02:27〕から 52.2 時間**空けたため rate-limit に触れず、マージ直後に反映された。readiness は `blocked=false mustHigh=0 verified=true`、articleHash は公開フリップ後の値と一致〔フリップは `published` 行1行のみで本文不変〕。なお #602 として先に sync PR を作ったが main が10コミット進んで古くなったため、現行 main から #613 として作り直してクローズした）

- 2026-09-04 zenn proceed-driven-agent-session-log https://zenn.dev/minewo/articles/proceed-driven-agent-session-log （queue #16、締切 2026-09-01 から3日遅れ。公開直前に PR #582 で全面改稿し、タイトルを「AIエージェントの事故・未遂を、次のガードレールに変える — 「進めて」で境界に触れた3件」へ変更。あわせて未反映だった F3〔「漏れ出る」が private リポジトリという実態より強い〕を解消し、`git stash drop` を「不可逆」と断定せず Git 公式ドキュメントの到達不能オブジェクト復旧に言及する形へ修正した。公開3点セット: 改稿 #582 → flip #583 main → sync #584 release/zenn でマージし deploy 発火。**HTTP 200・Zenn API 出現を確認済み**〔id 618265、published_at 2026-09-04 02:27 JST〕。主張境界レビューは一次情報5件すべて一致で Approve〔PR #257/#258 の実在、AGENT_LEARNINGS 2026-05-17/05-18/06-18 エントリと照合〕）
