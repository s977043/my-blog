...
- `[publishing]` **#15 (zenn) 締切 2026-08-31 以降**: 「AIセカンドブレインのマルチエージェント記憶」（`articles/ai-second-brain-multi-agent-memory.md`、`published: true`）。**main 段のみ完了（Zenn 未反映）**。レビュー成果物 `reviews/zenn/ai-second-brain-multi-agent-memory.md` — **2026-08-28 10:45 JST 実測**で `blocked=false mustHigh=0 verified=true loops=1 reviewedAt=2026-08-27T09:41:31Z`、`in-review` の遷移条件（blocked=false かつ mustHigh=0）を満たして `ready-to-publish` を経由し、2026-09-05 に `publishing` へ進めた。現行の articleHash は `7deb7226…`（下記フリップ後の値。reviews ファイルのヘッダと一致）。**締切を 8/31 以降にしていた当時の理由**（履歴）: 8/28 時点で `check:zenn-pace` が WARN(FAIL相当) 2 件、かつ #13 が deploy 未発火のため、先に #13 の決着を待つ必要があった。ゲート鮮度は reviewedAt からの経過ではなく articleHash 一致で判定する
