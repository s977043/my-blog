...
- `[requires-human]` **#13 (zenn) 締切 2026-08-29**: 「AIコードレビューを4層に分ける。River ReviewのJudgment Placement設計」（`articles/river-review-judgment-placement.md`、14,209字）。`reviews/zenn/river-review-judgment-placement.md` が `blocked=false mustHigh=0`（2026-08-26 実測）。flip PR #550（main、`published: true`）→ sync PR #551（release/zenn、2026-08-27 23:58 UTC マージ）まで完了済み。**しかし deploy が発火していない**。
  ...
  - 原因の第一候補は **Zenn rate-limit**。loop-maturity の公開（8/27 14:45 JST）から本記事の sync マージ（8/28 08:58 JST）まで **18時間13分**しかあいておらず、規約の 24 時間を下回っている
  ...
  - 人間の判断が要る点: ①さらに待って反映されるか（Zenn 側の遅延の可能性）②反映されなければ release/zenn への空 commit で再 deploy（2026-05-22 の river-reviewer-v033 と同型の復旧）③再試行は **前回公開から 24 時間以上**あけてから
...
- `[in-review]` **#16 (zenn) 締切 未設定**: 「『進めて』駆動のエージェントセッションログ」（`articles/proceed-driven-agent-session-log.md`、`published: false`）。レビュー成果物 `reviews/zenn/proceed-driven-agent-session-log.md` は 2026-08-28 10:45 JST 実測で `blocked=false mustHigh=0 verified=true loops=1 reviewedAt=2026-08-27T03:40:15Z` だが、**articleHash が STALE**（recorded=`d8b6578a116c…` / current=`6ebcf36006ec…`）。上の「状態遷移」表の定義では `check:publish-readiness` が stale なら `requires-human` に落ちるが、**別ワーカーが現在再レビューで STALE を解消中**のため `in-review` に留め置く。再レビュー完了後に articleHash 一致を確認してから `ready-to-publish` へ進めること
