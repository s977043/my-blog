...
### F7: 前作の名称とVerificationの説明精度

**重要度: medium / 対応済み**

最終レビューで2点を補正した。

- 冒頭の「AIエージェント開発を『Model + Harness』で考える」を、公開済み記事の実際の framing に合わせて「AI駆動開発を『Model + Harness』で考える」へ修正し、公開URLを追加
- 「実装した主体と、正しいと判定する主体が同じだから」を、主体の同一性そのものが問題だと読まれないよう、「Agent自身の自己申告だけでは実装とは独立した証拠にならない」へ修正

シリーズ上の用語整合性と、Test / Eval / Verificationの説明精度を上げた。
