...
対象記事 `Qiita/public/river-reviewer-agent-skills.md` を本文編集なしでレビューした。最優先は、River Review v0.33.0 時点の一次情報との事実整合性とした。

確認した一次情報:

- `/Users/user/Documents/GitHub/river-review/README.md`
- `/Users/user/Documents/GitHub/river-review/CHANGELOG.md`
- `/Users/user/Documents/GitHub/river-review/pages/reference/artifact-input-contract.md`
- `/Users/user/Documents/GitHub/river-review/skills/upstream/`
- `/Users/user/Documents/GitHub/river-review/runners/github-action/action.yml`
...
1. L223-L228: GitHub Actions の `uses` が現行 README と一致しない
   - 本文記述: `uses: s977043/river-review@v1`
   - 一次情報: README v0.33.0 のクイックスタートは `uses: s977043/river-review/runners/github-action@v0.28.0`。同 README は「最新リリース: v0.33.0」としつつ、Action は `@v0.28.0` などのリリースタグへピン留めする説明になっている。
   - 実物確認: `/Users/user/Documents/GitHub/river-review/runners/github-action/action.yml` は `phase` input を持つ composite action。README の path 付き Action が実物と一致する。
...
5. L257-L260: 「合っていない指摘の対応 Skill を見る」導入手順が現行 README の安全手順より粗い
   - 本文記述: `river-review@v1` を `phase: midstream` で動かし、対応する Skill を見て修正または追加する。
...
7. L194-L200: HITL の `Plan / Validate / Verify` は一次情報と整合
   - 本文記述: River Review は `Plan / Validate / Verify` の 3 ステップを前提。
...
| 対象 | 重複度 | 確認結果 |
| --- | --- | --- |
| Zenn: `articles/ai-dev-guardrail-plangate-river-reviewer.md` | 中 | 2 層ガード設計、PlanGate と River Review の役割分担、Plan / Validate / Verify が重なる。対象記事は River Review 体験寄りなので、Skill 改善ループと GitHub Actions 導入に寄せれば差別化できる。 |
...
| Qiita: `Qiita/public/plangate-ai-agent-governance.md` | 低〜中 | 体験談の構成、冒頭の課題提示、先に結論、導入前後の語り口が近い。ただし対象が PlanGate Metrics/Governance と River Review Skills で異なるため、トーン参考としては自然。 |

差別化の主課題: 対象記事は「River Review 単独の体験寄り」と書いているが、現状は note/Zenn で既に語った思想の再説明が多い。Qiita 版としては、README v0.33.0 の実物 workflow、`skills/upstream/` の実ファイル、Artifact Input Contract の入力一覧を使い、「このファイルを見れば始められる」記事へ寄せると公開価値が上がる。
...
- Low: `River Review` / `River Review` の表記が混在している。リポジトリ名・記事タイトル・本文で表記を統一したい。
...
- L223-L228: `uses: s977043/river-review@v1` -> `uses: s977043/river-review/runners/github-action@v0.28.0` に変更し、README と同じ `env: OPENAI_API_KEY`、必要なら `dry_run: true` を追加する [High]
...
- L249: 「本記事は River Review 単独の体験寄り」 -> 「本記事は GitHub Actions と Skill 実物を触る導入寄り」へ寄せ、Zenn 2 層ガード記事との差を明確化する [Medium]
- L274: 「PlanGate を 1 ヶ月運用してから」 -> 「PlanGate の plan / todo / test-cases が安定してから。目安として 1 ヶ月」 [Low]
- L41, L47, L119, L237, L253, L299: `River Review` / `River Review` -> 表記をどちらかに統一する。固有名は README に合わせるなら `River Review` [Low]
