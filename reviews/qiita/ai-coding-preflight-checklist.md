...
1. 姉妹記事 `claude-code-scope-creep-countermeasure.md`（PR #246 でmain反映済み、既に「関連記事」節と公開当日チェックリストを保有）への**対称的な相互リンクが本記事側に存在しない**（scope-creep へ＋ PlanGate Qiita記事 https://qiita.com/s977043/items/93027e02e962ec327c2f へ）
...
### 5. River Reviewer は本文非言及のため関連リンクから除外を検討

L227 `River Reviewer: https://github.com/s977043/river-reviewer` は本文に一切言及がない。scope-creep 側レビューでも同様に River Reviewer は「実装前の境界づくり」という記事スコープと不整合として除外判断され、scope-creep 本文・関連リンクには River Reviewer が無い（L224-227 は PlanGate / Growth Lab のみ）。

**問題点**: 本記事のスコープ（実装"前"の5項目計画）に、レビュー側ツールの River Reviewer は文脈が合わず唐突。scope-creep と非対称（あちらは除外済み）。

**提案**: scope-creep と揃え、関連リンクを PlanGate / Growth Lab の2本に絞り River Reviewer を除外する。
...
- [x] 外部リンクは記法上クリック可能（PlanGate / Growth Lab、River Reviewer は Recommended 5 で除外推奨）
...
4. （Recommended）River Reviewer を関連リンクから除外（scope-creep と整合）。タグ `開発プロセス`→`ClaudeCode` でシリーズ全5タグ一致。

### Qiita訴求観点での改善提案
- 検索流入の主軸は「AIコーディング 計画」「Goal Scope Non-goals」「Non-goals 書き方」。タイトル・本文で網羅済み。タグに `ClaudeCode` を足すと「Claude Code 計画」系も拾える。
- 同テーマ3記事（本記事 / scope-creep / plangate-governance id:93027e02e962ec327c2f）を相互内部リンクで束ねると、Qiita 内回遊と著者ページ関連表示が強化され、シリーズとして検索評価が積み上がる。後発の本記事に先行記事リンクを置く設計が回遊上もっとも効く。
