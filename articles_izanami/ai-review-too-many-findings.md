...
- 各章の「持ち帰る問い」には「答えが No なら最初の一手」を1行添える
...
- 量の測り方を1〜2行入れる: 直近20件程度のPRで、PR 1件あたりの指摘数と、対応・返信された指摘の割合（採用率）を数える。新しい章にはしない
...
- 主張: 件数を減らす第一段は、人が読む前に「形式として成り立たない指摘」を機械で落とすこと。ただし機械で検査できるのは形式と根拠の有無までで、「今回止めるべき指摘か」という意味の判断は機械では決まらない。この2つを分けて設計する
  - 用語を分ける: 「根拠の置き場所」（指摘の根拠として示す差分の行）と「問題の所在」（指摘している問題があるコード）
  - 機械で検査できる例（River Review の現在の実装の主な項目）: 根拠（Evidence）の記述があるか、根拠が差分に含まれるファイルを指しているか、一定の長さの修正提案があるか、重要度がその観点で宣言した上限を超えていないか、指摘の工程（phase）がファイル種別や観点の宣言と噛み合っているか
  - 機械で決まらないもの: 重要度の理由が妥当か、今回のPRで止めるべきか。これは人（またはレビュー方針）の判断領域として残す
  - 区別して書く: 根拠の置き場所が差分にない指摘は落とす。一方、問題の所在が差分の外（既存コード）であることは、それだけでは棄却理由にせず、範囲の情報として残す（扱いは第2章の並び順で決める）
  - 既製ツールの利用者の代わりの手段: ツールの指示ファイルやレビュー指示に「差分の行を根拠に示せない指摘は書かない」「意図が説明されている軽微な点は指摘しない」と書く
- 一次情報:
...
- 持ち帰る問い: 自分たちの AI レビューは、人に見せる前に何を落としているか。意味の判断まで機械に任せていないか
  - No なら最初の一手: レビュー指示に「根拠の差分行を示す」ことを必須として書く
...
- 主張: 全件を同じ重さで並べると読まれない。先頭に判定と重要度別件数を置き、Critical/Major だけ展開し、Minor/Info は件数付きで折りたたむ。折りたたみは削除ではない。第1章で残した「問題の所在が差分の外にある指摘」は消さず、並び順で後ろに回す
  - 既製ツールの利用者の代わりの手段: 重要度の高い指摘だけを PR の要約欄やチャット通知に流す。軽微な指摘は出力させないようレビュー指示で絞る
- 一次情報:
...
- 主張: 「人が全部読む」でも「AIが全部決める」でもなく、判定の出口を分けておく。本記事の独自点は次の2つに絞り、ESCALATE 条件の列挙は短くする
  ...
  - 「止めて直させる」と「人に判断を渡す」を区別する。River Review の現行実装例では、判定できないときは「通す」ではなく「止めて直させる」に倒し、人の判断が要る条件（人間承認が必要な変更など）は「人に判断を渡す」に分けている。これは一般則ではなく設計選択肢の一例として書く
  - 「通すが後で人が見る」で、読む量を減らしつつ人の目を残す
  - 本文に条件表の雛形（空欄）を置く: 行＝変更の種類、列＝4つの出口。読者が自チームで埋める
- 一次情報:
...
- 書き方: Issue の経緯は「1つの観測例」として1段落に縮め、主文は「抑制には失効・取り消し・基準変更時の見直しを設計する」に置く。Experimental な機能で観測した事例として書く。現在の挙動（期限・取り消しを見る、基準改訂時に止める判定は任意で有効化）と、修正までの経緯（#2418 → #2425 → #2430）を混同しない
- 一次情報:
...
- 導入順（手間の小さい順）: ①直近のPRで指摘数と採用率を測る → ②人に回す条件表を合意する → ③出させない指摘をレビュー指示に書く → ④黙らせた指摘の見直し日を決める
- 一次情報: 各章の再掲なし
