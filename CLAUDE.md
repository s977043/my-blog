...
報告に「すべて」「一度も」「全件」等の**全称命題**が含まれる場合、サンプリング照合では受理しない。**全数を確認してから受理する**。部分確認で全体を承認すると、誤った事実を前提に議論が進む（2026-08-19: 9 件中 3 件だけ照合して「すべて創作」を受理し、ユーザーへ誤った断言をした）。

検証手段による分業も意識する。同系統モデルを増やしても同じ手段しか取らなければ同じ穴が残るため、**論理・構成／数値の再実行／仕様の公式ドキュメント突合**を分けて依頼する（詳細は `@AGENT_LEARNINGS.md` 2026-08-20 エントリ）。
