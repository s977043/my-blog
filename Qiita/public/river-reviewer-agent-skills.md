...
そこで触ってみたのが [River Review](https://github.com/s977043/river-review/) という OSS フレームワークです。

考え方はシンプルです。

> プロンプトを磨くのをやめて、チームのレビュー知識を **Agent Skills（マニュアル付きの道具箱）** として明示化・バージョン管理する。

この記事では、River Review を実際に触ってみての体験と、AI レビューを「再現可能な資産」にする運用設計を、Qiita 向けに体験寄りでまとめます。

## 先に結論

River Review を入れて一番効いたのは、AI レビューの精度ではありません。
...
| フェーズ | PlanGate（実装前） | River Review（実装後） |
| --- | --- | --- |
...
## River Review とは（何で、何でないか）

[River Review](https://github.com/s977043/river-review/) は、OSS の **artifact-driven なコードレビューフレームワーク**です。
...
River Review のドキュメントで面白い概念が **「自由度の設計」** です。
...
River Review は **`Plan / Validate / Verify`** の 3 ステップを前提にしています。
  ...
  review:
    ...
    steps:
      - uses: actions/checkout@v6
        with:
          fetch-depth: 0
      - uses: s977043/river-review/runners/github-action@v0.28.0
        with:
...
River Review は単独でも動きますが、PlanGate と組み合わせると効きます。

- **PlanGate**（実装前）が `plan.md` / `todo.md` / `test-cases.md` を成果物として生成
- **River Review**（実装後）がその成果物を入力にして整合性を検査
...
つまり、PlanGate が「人間が計画を承認したか」を保証し、River Review が「実装が計画通りか」を保証します。
...
River Review をいきなり全 Skill 導入する必要はありません。

最初は、次の流れで十分試せます。

1. 既存リポジトリのどこか 1 つの PR で、`s977043/river-review/runners/github-action@v0.28.0` を `phase: midstream` + `dry_run: true` で動かす
...
特に最初の 2 つは、River Review のドキュメントでも繰り返し言われています。

## 明日から試すなら

River Review をいきなり全部導入しなくても、まずは次の問いから始められます。
...
その上で、River Review の [スキル作成チュートリアル](https://river-review.vercel.app/tutorials/creating-your-first-skill/) を見ながら 1 つだけ実装してみてください。
...
River Review は 2〜3 番目を埋めるための OSS フレームワークです。
...
- [River Review GitHub Repository](https://github.com/s977043/river-review/)
- [River Review ドキュメント (Vercel)](https://river-review.vercel.app/)
- [スキル作成チュートリアル](https://river-review.vercel.app/tutorials/creating-your-first-skill/)
- [アーキテクチャ解説](https://river-review.vercel.app/explanation/river-architecture/)
- [Artifact Input Contract](https://github.com/s977043/river-review/blob/main/pages/reference/artifact-input-contract.md)
- 関連記事:
  - Zenn: [AI 駆動開発の 2 層ガード設計：PlanGate と River Review で実装前後を守る](https://zenn.dev/minewo/articles/ai-dev-guardrail-plangate-river-reviewer)
