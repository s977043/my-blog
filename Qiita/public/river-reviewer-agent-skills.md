...
**「PR の差分」だけを AI に投げるのではなく、`plan` / `diff` / `test-cases` / `junit` などのアーティファクトを入力にして、Skill Registry から状況に応じた Skill を選んでレビューする。**
...
レビュー結果に違和感があれば、対応する Skill の `prompt.md` を直し、`tests/` に再現フィクスチャを足し、eval で回して比較する。一連の改善ループが、コードと同じワークフローで回せます。
...
実際に試すなら、GitHub Actions の `v1` タグで最小構成を組めます。
...
`phase` を切り替えるだけで、上流（設計） / 中流（実装） / 下流（QA）に対応する Skill が自動選択されます。
...
詳細な 2 層ガード設計は [Zenn 記事「AI 駆動開発の 2 層ガード設計」](https://zenn.dev/minewo/articles/ai-dev-guardrail-plangate-river-reviewer) にまとめています。本記事は River-Reviewer 単独の体験寄りに絞っています。
...
1. 既存リポジトリのどこか 1 つの PR で、`river-reviewer@v1` を `phase: midstream` で動かす
2. 出力された `findings` を見て、**ピントが合っている指摘 / 合っていない指摘**を分類する
3. 合っていない指摘について、対応する Skill を 1 つだけ選んで `prompt.md` を見る
4. Skill 側の表現を直すか、新しい Skill を 1 つ追加する
5. 同じ PR で再実行して改善を確認
...
- **PlanGate と組み合わせるなら、まず PlanGate を 1 ヶ月運用してから**。どちらも一度に入れると認知負荷が大きい
