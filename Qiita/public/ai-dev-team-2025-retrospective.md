...
この記事では、2025年にAIコーディングエージェントを使いながら考えたことを、個人開発とチーム開発の間にある運用設計の話として振り返ります。
...
2025年の途中までは「AIが速く書けること」に意識が寄りすぎていました。実際に困ったのは実装速度ではなく、あとから差分を読んだときに「なぜこの変更が入ったのか」「どこまでが今回の依頼だったのか」を説明し直す時間でした。
...
PoCとして「一度うまく動いた」と、チームの開発プロセスとして「繰り返し安全に回る」は別物です。2025年にいちばん大きく変わったのは、AIの使い方そのものより、この差を強く意識するようになったことでした。
...
これは、AIにとってのメモリを「長く覚えてくれる機能」として期待するより、参照すべき情報を明示的に置く、という考え方に近いです。AIが何を見て、何を見ずに、どの前提で進めるのか。そこをファイルで制御するほうが、チーム開発では扱いやすいと感じました。
...
AIレビューでも同じことが起きます。指摘が多すぎると、重要な論点が埋もれます。そこで、自分はレビュー観点もざっくり3つに分けて考えるようになりました。

- Must Fix: 放置すると不具合、仕様逸脱、事故につながるもの
- Should Fix: 今すぐ壊れないが、保守性や拡張性を落とすもの
- Nice to Have: 今回止める理由は弱い改善提案

「指摘する」と「マージを止める」は別です。この境界を先に決めておくと、AIのレビュー結果も人間が扱いやすくなります。
...
PlanGateで大事にしているのは、次の4つです。

- 止める: 計画が承認されるまで実装しない
- 追える: 判断や前提をあとから確認できる
- 戻せる: スコープ外に広がったときに切り戻せる
- 改善する: レビューや振り返りの学びを次回のルールに戻す

AIエージェントをチームに入れると、「どれだけ賢く書けるか」よりも、「止める・追える・戻せる・改善できるか」が効いてきます。
...
もうひとつ効いたのは、レビューで得た学びをその場のコメントで終わらせないことです。

たとえば、AIレビューが毎回同じズレ方をするなら、それは「AIがいまいちだった」で終わらせるより、チームルール候補として扱ったほうがいいです。採用する指摘、却下する指摘、判断理由を残しておくと、次回のプロンプトやチェックリスト、場合によっては lint や CI に戻せます。

振り返りも同じです。「次は気をつけよう」で終わると、人間の注意力に依存します。できれば出口を次の3つに分けたい。

- ドキュメントに残す
- 自動ガードにする
- ツールや作業手順を変える

AIと働くほど、こうした小さな運用改善の積み重ねが効いてきます。
...
- レビューや振り返りの学びを、ルール、CI、チェックリストに戻す
...
そして2026年は、その整え方をもう少し具体的な開発プロセスとして育てていきたいと思っています。AIを使うほど、開発プロセスの曖昧さは表に出ます。だからこそ、AI導入は単なる効率化ではなく、チームの仕事の進め方を見直すきっかけにもなるはずです。
