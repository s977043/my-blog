...
`DESIGN.md` は「読む順番」と「正本の場所」を固定するための軽量な入口ファイルです。詳細を書き込む辞書ではなく、AI と人が最初に開く索引として置きます。これだけで、デザインレビューの論点ぶれと AI への前提説明の繰り返しが減ります。

:::note info
本記事は、Zenn に掲載した [DESIGN.md 導入ガイド: AI実装のための入口・契約・検証をどう整えるか](https://zenn.dev/minewo/articles/design-md-guide-and-adoption-log) を Qiita 向けに再構成したものです。
:::
...
| 時期       | 変化                                                             | 意味                                                                     |
| ---------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------ |
| v0.7.0     | Penpot 統合、デザイントークン CI、コンポーネントマッピングを強化 | デザインツールと実装の接続を作り始めた段階                               |
