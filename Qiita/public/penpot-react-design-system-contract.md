...
- デザインツールと React 実装のズレを減らしたいフロントエンドエンジニア
- AI に UI 実装を任せる前提でデザインシステムを整えたい人
- Penpot や Figma を使っているが、実装との同期に頭を悩ませている人

## 先に結論

Penpot と React は「同期する」のではなく **同じ契約に従わせる** と運用が安定します。契約の正本は `tokens/*.json` と `component-map.json`。ツール（Penpot）はあくまで作業の場で、ファイルが正しさの基準です。AI 実装は、見た目の指示ではなく契約への参照で動かします。
...
- 入口編（2作目）: [DESIGN.md 導入ガイド: AI実装のための入口・契約・検証をどう整えるか](https://qiita.com/s977043/items/1ce6753867f4b166d74b)
- 続編（3作目）: [Open Designでデザイン品質を上げる：Penpot契約運用とDESIGN.mdの続編](https://qiita.com/s977043/items/af06444b664553ecdc8a)
