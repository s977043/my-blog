...
high-risk 以上では、承認の前に別の AI のレビューが入ります。今回は条件付き承認 → 修正 → Mode を上げたので別の AI のレビュー → 人の最終承認、の順です。
...
計画の Mode は standard です。しかし第 4 章の数える目安では、変更ファイル 6 件（high-risk は 6〜15 件）と受入基準 7 件（同 6〜10 件）のどちらも high-risk の範囲に入ります。重い方を採るので、この計画は high-risk として扱います（第 4 章）。
...
このサンプルの初稿は、目標、やらないこと、作業の順序、受入基準とテストケースの対応は筋が通っています。足りないのは前提と止まる条件の書き方で、計画の大筋ではないので、筆者の作例では条件付き承認とします。ただし Mode を上げたので、別の AI のレビューを通してから人が承認します。
