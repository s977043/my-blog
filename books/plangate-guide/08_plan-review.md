---
title: "1 つの計画を通してレビューする"
---

第 3〜7 章で、計画に入れる要素を一つずつ見てきました。本章では、それらを 1 つの計画にまとめて読み、レビューしてみます。
...
今の形式で書いた計画の例は、第 9 章の作例（注文一覧 API の絞り込み）を見てください。
...
flowchart TD
    ...
    C --> D["指摘・条件付き承認<br/>（人）"]
    D --> E["修正<br/>（AI）"]
    E --> G["別の AI のレビュー<br/>（high-risk 以上）"] --> F{"最終承認<br/>（人）"}
```

high-risk 以上では、承認の前に別の AI のレビューが入ります。今回は、人の指摘を条件付き承認として受けて修正し、Mode を上げたので別の AI のレビューを通してから、人が最終承認します。
...
計画の Mode は standard です。しかし第 4 章の数える目安で数え直すと、変更ファイル 6 件は high-risk の範囲（6〜15 件）に入ります。受入基準 7 件も、high-risk の範囲（6〜10 件）です。重い方を採るので、この計画は high-risk として扱います（第 4 章）。

4 点とも、足りないのは前提・止まる条件・設計判断の書き方と Mode です。目標、やらないこと、作業の順序、受入基準とテストケースの対応は筋が通っていて、計画の大筋は有効です。そこで筆者の作例では、ここで条件付き承認（大筋は有効だが、直す点がある）とし、指摘を反映してもらいます。
...
修正した計画は Mode を high-risk に上げたので、別の AI のレビューを通してから、人が最終承認します。PlanGate の承認には 3 つの結果があります。

| 結果 | 意味 | 次に進む先 |
|---|---|---|
| APPROVE（承認） | 計画に問題はない | 実装を始める |
| CONDITIONAL（条件付き承認） | 大筋は有効だが、直す点がある | 指摘を反映し、簡易なセルフレビューを通してから実装を始める |
| REJECT（差し戻し） | 根本的な問題がある | 計画を作り直す |

本章の作例では、「4. 人の指摘」で条件付き承認として指摘を反映しました。ただし Mode を high-risk に上げたため、承認の前に別の AI のレビューが必要になります。そのため通常の条件付き承認の流れとは違い、別の AI のレビューを通したうえで、ここで改めて承認（APPROVE）とします。

承認されるまで、実装には 1 行も進みません。承認されたあとで計画を書き換える必要が出たら、もう一度この承認を通します。

次章では、第 3〜7 章で見た欄を 1 枚の骨組みにまとめ、自分の計画を書き始めます。
