...
抽象論より、実際の挙動を見るのが早いです。たとえば C-3 承認を取らずに実装へ進もうとすると、EH-2 が次のように警告します（default モード）。対象のタスクは `PLANGATE_HOOK_TASK` か、編集するファイルのパスに含まれる `TASK-XXXX` で決まります。どちらも無いと EH-2 は判定をスキップするので、第1章の注記のとおりタスクを指定しておきます。
...
> 例外として **EH-9（委譲先の勝手な commit / push）だけは default でも block** です。誤検知が起きにくく、かつ起きると影響が大きい（意図しない commit）ため、初期から強制されます。リスクの高い不変条件は最初から strict、という設計です。
