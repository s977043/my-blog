...
`plan.md` を作らずに production code を編集しようとすれば、EH-1 が警告します。
...
そして承認した計画をこっそり書き換えれば、前章で触れた EH-3 が `plan_hash` の不一致を検知します。「承認した計画と、いま動いている計画が違う」という最も危険な状態を、機械が捕まえます。

いずれの違反も、append-only の監査ログに記録されます。

```text
<ISO8601 UTC>	VIOLATION	check-c3-approval	TASK-0001	approvals/c3.json not found or c3_status != APPROVED
```
...
| モード | 環境変数 | 挙動 | 使いどころ |
|--------|----------|------|------------|
| **default**（推奨初期値） | なし | 違反は warning のみ。`continue:true` でブロックしない | 導入初期、観察フェーズ |
| **strict** | `PLANGATE_HOOK_STRICT=1` | 違反でブロック（exit 1） | 本番運用、CI、重要タスク |
| **bypass** | `PLANGATE_BYPASS_HOOK=1` | 常時 pass。監査ログには必ず記録される | 緊急対応、誤検知の一時回避 |
...
> Next.js App Router を題材にした AI-driven TDD の具体的な最小ループは、本書とは別に詳しく書いた記事があります → [AI 駆動開発の成果をどう測るか: 速度ではなく手戻り率を見る](https://zenn.dev/minewo/articles/ai-driven-dev-metrics)
...
- **ローカル Hook（PlanGate）** — AI がコードを書く**その瞬間**に、計画からの逸脱を検知する（事前・高速・きめ細かい）
...
> 🔗 実際に Hook が警告する様子を自分の手で試す → 第 1 章「クイックスタート」へ戻る / [PlanGate を clone](https://github.com/s977043/PlanGate)
