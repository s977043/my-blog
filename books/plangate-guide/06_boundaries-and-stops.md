---
title: "AI が決めてよい境界と、止まる条件を書く"
---

前章では、計画の前提を確かめる話をしました。本章では、計画を実行する AI に「どこまで自分で決めてよいか」「どこで止まるか」を伝える方法を扱います。

人間のチームなら、新しく入ったメンバーにも「この辺は自分で判断していい」「ここは必ず相談して」という暗黙の線引きがあります。AI にはその暗黙の線引きが伝わりません。書いていないことは、自分で決めてよいことだと受け取ります。

## やらないことと、触らないファイル

第 3 章で書いた Out of scope は、ここでも役に立ちます。PlanGate の計画テンプレートでは、スコープの境界として次のものを書きます。

- **やらないこと**: 今回扱わない機能や改善
- **変更を禁止する範囲**: 触らないファイル、ディレクトリ、設定
- **追加を禁止する依存**: 新しく入れてはいけないライブラリ。入れてよい場合は承認の条件

サンプルのユーザー登録 API では、「既存の認証ミドルウェアは変更しない」と書かれています。仮に登録のリクエストがミドルウェアに横取りされると分かったら、AI はミドルウェアのほうを直したくなるかもしれません。そうされると、ログインなど登録と関係ない処理の挙動まで変わるおそれがあります。触らないと書いてあれば、AI はそこで止まって相談することになります。
...
実装の途中で、計画どおりに進められない事態は必ず起きます。そのとき AI がどう振る舞うかを、事前に決めておきます。PlanGate のテンプレートは、二種類の条件を分けて書くようにしています。
...
第 4 章で触れた「今は作らない将来の拡張」も、AI が決めてよい境界の一つです。

AI は、将来役に立ちそうな仕組みを先回りして作りたがります。拡張しやすくするための抽象化や、今は使わない設定項目です。これを「作らない」と書いておかないと、AI にとっては「作ってもよい」ことになります。

計画に書くのは、「何を作らないか」と「なぜ今は作らないか」です。理由まで書いておけば、後で本当に必要になったときに、なぜ見送ったのかを振り返れます。
