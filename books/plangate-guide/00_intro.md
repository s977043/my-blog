...
この本で見る全体像は、次の流れです。まず **Plan で何を作るかを固め、Approve で人間が承認し、Exec で逸脱を止め、Verify / Scale で学びを次の計画へ戻す**、という流れです。

```mermaid
flowchart LR
    P[Plan<br/>計画の精度] --> A[Approve<br/>C-3 承認]
    A --> E[Exec<br/>守らせる強制力]
    E --> V[Verify<br/>守られた証明]
    V --> S[Scale<br/>継続改善]
    S -. 学びを次の Plan に戻す .-> P
```
...
- **第 1 章 クイックスタート** — 10 分で「承認なし実装が止まる」体験
