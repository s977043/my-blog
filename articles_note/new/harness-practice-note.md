...
PlanGate と River Review は開発プロセス側に置いた仕組みです。一方、以下のブログ運用は、私が事故と改善を最も多く観測できた実験場でした。

ブログ運用のリポジトリには、main への直コミットをブロックする pre-commit hook、公開前の整合を見る20本のチェックスクリプト、記事の最終確認を回す3本のWorkflowを積みました。

本数に意味はありません。口頭で守っていた約束を、誰が実行しても同じ結果になる形へ置き換えた回数です。
...
この境界を意識するきっかけの1つが、[Darwin Gödel Machine（DGM）の論文](https://arxiv.org/abs/2505.22954)でした。DGMは自分自身のコードを書き換えながら改善するエージェントで、SWE-bench Verifiedの200タスク部分集合では20.0%から50.0%、Polyglotでは14.2%から30.7%へ性能を伸ばしています。
