...
PRONI株式会社のプロダクト開発部で、プロダクト開発に携わっているみね（[@mine\_take](https://x.com/mine_take)）です。
...
この部の発信では、CTOの竹澤さんとエンジニアリングマネージャーの岸本さんが「最大のボトルネックが『何を解くかを定義すること』へと移ってきた」と話しています（[AI活用によって劇的に変わった、エンジニアとゴール（成果）の距離感とは？](https://note.proni.co.jp/n/nb1f7d2dce412)）。本記事は、その「何を解くかを定義する」を、エージェントへ実装を任せる場面でどう具体化するかという話でもあります。
