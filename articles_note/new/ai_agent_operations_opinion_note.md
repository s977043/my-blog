...
```mermaid
flowchart TD
    Start[依頼を受ける] --> Plan[計画を立てる]
    Plan --> Act[ツールを実行する]
    Act --> Check{結果を評価する}
    Check -->|成功| Log[判断と結果を記録する]
    Check -->|失敗| Recover[再試行・人間へエスカレーション]
    Recover --> Plan
    Log --> Next[次の処理へ進む]
```
...
```mermaid
flowchart LR
    User[利用者・業務文脈] --> Policy[権限・目的・鮮度の判定]
    Policy --> Retrieval[検索 / 参照]
    Retrieval --> Memory[(Memory / Knowledge)]
    Policy --> Deny[参照しない判断]
    Retrieval --> Agent[AIエージェント]
    Agent --> Audit[参照理由と判断履歴]
```
