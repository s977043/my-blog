...
この記事では、AIコーディングで実装は速くなった一方、品質や判断の統制に難しさを感じているエンジニアに向けて、「何を作るか」「どう確かめるか」「どう学びを残すか」という観点から整理します。最後に、次の依頼から使える6つの確認質問へまとめます。
...
使い続ける中で、計画の承認だけでは防げない失敗も経験しました。Zennの公開ブランチを同期したときは、mainとの乖離を抱えたブランチ全体をマージしようとして、対象記事とは無関係な4ファイルで衝突が起きました。

同期する範囲と停止条件を事前に固定していなかったため、作業自体は続けられても、機械的に衝突を解消すれば別の記事の公開状態を意図せず古い状態に戻してしまう危険がありました。

それ以来、同期対象を単一ファイルに限定し、対象ファイル以外に差分がないことを確認してから完了とする運用に変えています。
...
## 経験を整理すると、設計対象はコードの外に広がった

PlanGateでの失敗と改善を振り返ると、設計している対象はコードだけではありませんでした。作業前後の見落としを減らすため、現在は次の6観点で捉えています。

### 1. Problem / Intent（何を、なぜ作るか）

誰の何を変えたいのか。なぜ今取り組むのか。何を作らないのか。成功と撤退をどう判断するのか。

ここが曖昧なままでは、後続の設計が精密でも価値にはつながりません。

### 2. Contract / Context（何を渡し、何を守るか）

AIへ渡す目的、制約、既存設計、参考情報、非対象、受入条件、出力形式を決めます。

単に長いプロンプトを書くのではなく、作業の契約を明示する感覚に近いです。

### 3. Harness / Runtime（どこで、何を使わせるか）
