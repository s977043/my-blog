...
普段から仕事や個人開発で、Coding Agentを使ったAI駆動開発を試しています。

最近、その中で自分の見方がひとつ変わりました。

**Agentが失敗したとき、最初にModelの名前を見なくなりました。**
...
以前なら、まずModelの能力を疑っていたと思います。

もちろん、推論能力そのものが原因のこともあります。

でも、こうした失敗を一つずつ見ていくと、完了条件、Context、Tool、途中の状態、検証の仕組みなど、Modelの外側に原因があることも多いと感じるようになりました。

そして、自分が改善しているのはModelそのものより、**Modelが仕事をするための外側の仕組み**なのではないか、と考えるようになりました。

その仕組みを考えるうえで、最近かなりしっくりきている言葉があります。
...
前の記事で考えたのは、BuildをEvidenceへ変え、次の意思決定につなげるProductのLearning Loopでした。

今回は、そのLoopを支える側を考えたいと思います。

**Coding Agent自身が失敗から学び、改善し続けるためには、何を設計すればよいのか。**
...
Toolを間違えたなら、疑うべきはToolの入力形式や説明、どのToolを選ぶかというRouting、そしてContextです。

検証を飛ばしたなら、作業手順（Workflow）や承認・停止の境界（Gate）を疑います。

同じ処理を繰り返すなら、疑うのは繰り返し処理の設計とStop Conditionです。

危険な操作をしそうになったなら、権限（Permission）や隔離された実行環境（Sandbox）を疑います。

何が起きたのか分からないなら、疑うべきは実行を観測できる仕組みです。

修正したあと本当に良くなったのか分からないなら、同じ基準で結果を比べる評価（Eval）を疑います。
...
本番で一度起きた失敗を、単なる障害や「AIだから仕方ない」で終わらせません。

次にやるのは、失敗を分類して、再現できる評価ケースへ変えることです。

そのうえでHarnessを変更し、同じ失敗が再発しないか回帰評価を通します。
...
同じAgentに、実装して、レビューして、修正して、最後に自分で正しいと承認させるより、作る役割と判定する役割を分けたほうが確実です。
...
そのために、Model、Prompt、Skill、Harness、EvaluationのVersion、使ったTool、Cost、結果、失敗の種類を、1回の実行ごとに追えるようにしておく必要があります。
...
自分自身、以前はPlanGate、River Review、Skills、Memory、Eval、Retrospectiveを、それぞれ別の仕組みとして見ていました。

今は、それらを**Agentが仕事をし、失敗から学ぶためのHarness**として一つのSystemで見るようになりました。

良いModelを選ぶことは、これからも重要です。

ただ、自分は今、AI駆動開発を単に「どのModelを使うか」ではなく、**Modelの能力を、どんなHarnessで仕事として成立させるか**まで含めて考えたいと思っています。

Harnessも、一度作って終わりではありません。

失敗を観測する。原因を分類する。Harnessを変える。評価する。そして、不要になった仕組みは削る。

自分が今作りたいのは、そんな**失敗から学び続けるAI駆動開発のSystem**です。

```text
Agent = Model + Harness
```

失敗をModelのせいだけで終わらせず、Systemとして説明し、改善できる状態を作る。

**今のところ、自分はそれがAI駆動開発の重要なEngineeringになると考えています。**

そして、この先でもう一つ考えたいことがあります。

HarnessがAgentを「どう動かすか」を支えるものだとしたら、次に考えたいのは、**Agentに「何が正しいか」をどう渡すのか**です。

仕様、Test、Domain Design。

AIがコードを書くほど、こうしたCoding以外のEngineeringがなぜ重要になるのか。次の記事では、そこを掘り下げたいと思います。
