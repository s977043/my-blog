...
4. **reviews 成果物（`reviews/zenn/$1.md`）は記事の公開/改善 PR に同梱しない**。レビューは「ある時点のスナップショット」で、その後の改善で必ず陳腐化する（記事PRに混ぜると bot に「記事と乖離」と指摘され手戻りになる。AGENT_LEARNINGS 2026-06-07）。記録する場合は**別 PR / 別ライフサイクル**にする。
