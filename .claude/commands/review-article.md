---
description: 指定した記事を3ペルソナでレビューし、reviews/<slug>.md を生成してPRを作成する
argument-hint: <article-slug> (articles/ 配下のファイル名 .md 抜き)
---

# /review-article

指定した記事を3ペルソナ視点でレビューし、`reviews/<slug>.md` を生成してPRを作成する。
...
3. `article-reviewer` エージェントを起動し、以下を委譲:
   ...
   - `reviews/$1.md` を生成（既存があれば上書き）
   - フォーマットは `.claude/agents/article-reviewer.md` 準拠

4. コミット
   ```bash
   git add reviews/$1.md
...
## ガードレール
- 既存 `reviews/$1.md` を上書きする場合は差分を提示して確認
