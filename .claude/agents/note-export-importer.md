...
2. **新規記事のインポートWXR生成** — `articles_note/new/<slug>.md` から単一記事WXRを `articles_note/build/<slug>.xml` に出力
...
3. 本文中のローカル画像（`../assets/...` 等）を列挙し、**ユーザーに対処方針を確認**:
   - A) 画像をGitHub Raw等にpush → https URLに書き換えて再変換
...
4. スクリプトを実行:
   ...
   ```
   → `articles_note/build/<slug>.xml` が生成される
5. 生成されたWXRの構造を検証（**必須**。wp:* 欠落がないか確認）:
   ```bash
   python3 .claude/skills/note-export-import/scripts/verify_wxr.py articles_note/build/<slug>.xml
   ```
   → エラーがあればユーザーに報告し、md_to_wxr.py の出力を確認してから再実行
6. 生成ファイルのサイズを確認 (20MB以下か)
7. ユーザーへ次のアクションを案内:
   - note管理画面 → プロフィール → 自分の記事 → インポート → WXR選択
   - `articles_note/build/<slug>.xml` をアップロード → インポートを開始
...
### WXR生成後
```
✓ articles_note/build/<slug>.xml (約 XX KB)
警告: ローカル画像 N件（インポート後に手動貼り直し必要）
次のステップ:
  1. note管理画面 → 自分の記事 → インポート
  2. WXR選択 → build/<slug>.xml をアップロード
