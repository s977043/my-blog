...
- 記事タイプに応じた想定読者
- 事実・リンク・数値の確認
- H2が論点カタログになっていないか、上位の読者軸へ束ねられるか
- 専門語の初出説明と、2回目以降の説明重複
- 同じLoop / 図 / 主張の再掲
- 終盤が複数の結論や次テーマに分散していないか

レビューは2段階で行う。

1. **内容・構造Pass**: 中心主張、事実、根拠、読者導線を確認
2. **密度・削減Pass**: 新しい論点を追加せず、重複説明、用語密度、不要なH2、固有名詞の早すぎる初出、終盤の競合を削れるか確認

問題が少ない記事に件数合わせの指摘を作らない。内容が足りている記事では、追加より圧縮を優先する。指摘0件でも総合評価と未検証事項を残す。
...
- [ ] H2本数を固定値で評価せず、読者が追う軸と見出しの役割重複を見る
- [ ] 用語集を自動追加せず、初出で一度だけ説明する形を優先する
- [ ] 内容レビュー後に「削減だけ」を目的としたPassを行う
