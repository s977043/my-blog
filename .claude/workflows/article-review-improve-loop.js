export const meta = {
  ...
  // 注: Loop フェーズは loops（1..5）で動的に増減する。ここでは代表値として 3 周ぶんを宣言。
  phases: [
    { title: 'Extract' },
...
const finalMustHigh = mustHighCount(lastReview)
...
return {
  ...
  finalBlocked: publishBlockers != null && publishBlockers > 0, // must が残る＝公開不可
  claimPreservedAll: improvedHistory.length > 0 && improvedHistory.every(h => h.improve.claimPreserved),
