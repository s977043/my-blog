...
### 2026-04-18 — WXR(note公式形式)の著者フィールドと CDATA エスケープ規則 [Convention]

**観察**: `md_to_wxr.py` を note.com 公式エクスポート形式に寄せる改修で、author の扱いと CDATA 衝突で複数回手戻りが発生した。特に以下が非自明:

1. `<dc:creator>` には **author_login** (≒システム上のID `mine_unilabo`) を入れるのが WXR 慣例で、表示名はここに入れない
2. 表示名は `<wp:author><wp:author_display_name>` で指定する（channel 直下の `<wp:author>` ブロック内）
3. CDATA 衝突は `<title>` と `<content:encoded>` の**両方**で起きうる。タイトルに `]]>` を含む記事があると XML が途中終了する

**対策/学び**:
- WXR を手書きするスクリプトでは `dc:creator` = login ID、`wp:author_display_name` = 表示名と分けて扱う（noteが両方を参照する）
- `]]>` 分割処理（`]]]]><![CDATA[>`）は body HTML だけでなく **title にも** 適用する。共通化するなら lambda / 関数として抽出する
- 時刻を複数箇所で扱う場合（pubDate と出力ファイル名の ts 等）、**全箇所で同じ TZ** に統一すること。`datetime.now()` と `datetime.now(timezone(timedelta(hours=9)))` を混在させると実行環境で揺れる
- gemini-code-assist / sentry-seer の line-level comment はピンポイントかつ根拠が明確なので、ローカル検証（`xmllint --noout` + サンプル実行）で裏取りしてから一気に反映するのが効率的

**根拠**: PR #57 (dad8236)、gemini-code-assist / sentry-seer のレビュー、commit `b8044f5`
