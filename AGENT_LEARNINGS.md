...
1. `<dc:creator>`には **author_login** (≒システム上のID `mine_unilabo`)を入れるのがWXR慣例で、表示名はここに入れない
2. 表示名は`<wp:author><wp:author_display_name>`で指定する（channel直下の`<wp:author>`ブロック内）
3. CDATA衝突は`<title>`と`<content:encoded>`の**両方**で起きうる。タイトルに`]]>`を含む記事があるとXMLが途中終了する

**対策/学び**:
- WXRを手書きするスクリプトでは`dc:creator` = login ID、`wp:author_display_name` = 表示名と分けて扱う（noteが両方を参照する）
