...
**観察**: `river-review-judgment-placement` の内部リンク整備で「文脈内配置は末尾リンク集より評価される」という一般的な SEO 指針を見つけ、これを根拠に AGENTS.md の「note / Qiita へのリンクは末尾リンク集のみ」という規約を緩めるべきかを検討した。調べ直したところ、この指針は**内部リンク（同一ドメイン）についてのもの**で、Zenn 記事から note / Qiita へのリンクは**外部リンク**にあたり扱いが逆だった。内部リンクは評価をサイト内で再分配するが、外部リンクは評価を外へ渡すため、外部リンクは記事あたり 1〜3 本に絞るのが一般的な目安になる。既存規約は緩める必要がなかった。

**気づき**: 本当の穴は別にあった。**外部リンクにだけ厳しい規約があり、SEO 効果が実際にある Zenn 記事間の内部リンクは無規約**という非対称が残っていた。実態として、内部リンクを持つ既存 4 記事すべてが `## まとめ` 以降にしかリンクを置いていない。「規約があるほう」ばかり整備され、「規約がないほう」が放置されていた。

**対策/学び**:
- リンクの配置指針を検討するときは、まず **内部（同一ドメイン）か外部（他プラットフォーム）か**を切り分ける。切り分ける前に一般論を当てはめない
- AGENTS.md 表現規約に「記事内の同一プラットフォーム参照（Zenn ↔ Zenn / Qiita ↔ Qiita）」節を新設。本文中の文脈内リンク 2〜4 本・説明的アンカーテキスト・`/articles/<slug>` 相対形式・双方向の張り返しを明文化した
- クロスプラットフォーム参照の節には「この指針は内部リンクの話なので本規約を緩める根拠にならない」と理由を追記し、同じ検討を繰り返さないようにした
